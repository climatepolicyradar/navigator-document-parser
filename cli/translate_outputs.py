...
def identify_translation_languages(document: ParserOutput, target_languages: Set) -> Set:
    ...
    if document.languages and len(document.languages) == 1:
        ...
        _LOGGER.debug(f"Removing {minus if minus is not None else None} from {target_languages}.")
...
def translate_parser_outputs(task_output_paths: Sequence[Union[Path, CloudPath]], redo: bool = False) -> None:
    ...
    for path in tqdm(task_output_paths):
        ...
        try:
            ...
            _LOGGER.debug(f"Successfully parsed {path} from output dir during translation processing.")
        except FileNotFoundError:
            _LOGGER.error(f"Could not find {path} in output dir during translation processing.")
            continue
    ...
    for target_language in target_languages:
        try:
            ...
            if output_path.exists() and not redo:  # type: ignore
                _LOGGER.info(f"Skipping translating {output_path} because it already exists.")
                continue

...
def polygon_to_coords(polygon: list[Point]) -> list[tuple[float, float]]:
    ...
    """
    # FIXME: Check that these are the co-ordinates we want to form the box
    #  (are we using bottom right, top left elsewhere?)
    return [(polygon[1].x, polygon[1].y), (polygon[3].x, polygon[3].y)]


def convert_to_text_block(paragraph_id: int, paragraph: DocumentParagraph) -> PDFTextBlock:
    ...
    return PDFTextBlock(
        ...
        text_block_id=paragraph_id,
        language=None,
        type=paragraph.role if paragraph.role is not None else 'Ambiguous',
        type_confidence=0.0,
    )
...
def merge_responses(pages: Sequence[PDFPage]) -> AnalyzeResult:
    ...
    [
        page_merged.tables.append(page.extracted_content.tables)
        for page in pages[1:]
    ]
